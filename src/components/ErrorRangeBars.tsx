...
import { SeriesRangePointType } from '../types';
    ...
    const pointBars = data.map((point, i) => {
      ...
      return (
        <PointBars // eslint-disable-next-line react/no-array-index-key
          key={`ErrorBars-${i}`}
          origin={{ x: xScale(point.x), y: yScale(point.y1) }}
          bottom={yError ? yScale(point.y1 - yError[0]) : null}
          top={yError ? yScale(point.y1 + yError[1]) : null}
